...
import { Registration, SkillSeeker } from 'src/app/api/flexcub-api/models';
...
type rateElements = RateElement[];

var ELEMENT_DATA: RateElement[] = [];

@Component({
  selector: 'app-add-new-skill-seeker',
  templateUrl: './add-new-skill-seeker.component.html',
  styleUrls: ['./add-new-skill-seeker.component.scss'],
})
export class AddNewSkillSeekerComponent implements OnInit {
  editable: boolean = true;
  registrationAccess:boolean;
  // seekerStatus:boolean
  isNew: boolean = true;
  ...
  ngOnInit(): void {
    // this.seekerStatus = this.user?.accountStatus;
    // console.log(this.seekerStatus);
    ...
    this.activateRoute.queryParams.pipe(filter((param: Params) => !!param && !!param?.type)).subscribe((param: Params) => {
      ...
      if (param?.type === 'Requirement') {
        this.activeTab = 'Requirement';
      }
    });
  ...
  get businessDomain() {
    return this.form.get('businessDomain');
  }
  ...
  getQueryParams(): void {
    this.activateRoute.queryParams
      ...
      .subscribe((param: Params) => {
        ...
        this.skillSeekerBasicDetail(this.editId);
  ...
  skillSeekerBasicDetail(id: number) {
    this.superAdminservice.skillSeekerBasicDetail(id).subscribe((data: SkillSeeker) => {
      if (data?.primaryContactFullName) {
        var names = data?.primaryContactFullName?.split(' ');
        var firstName = names[0];
        var lastName = names[1];
      }
      this.registrationAccess=data?.registrationAccess
      this.form.get('businessName').patchValue(data.skillSeekerName);
      this.form.get('businessDomain').patchValue(data.ownerSkillDomainEntity?.domainId ?? null);
      ...
      this.form.get('alternativeemailaddress').patchValue(data.secondaryContactEmail);
      this.form.get('alternativephonenumber').patchValue(data.secondaryContactPhone);
      ...
      this.form.get('zipcode').patchValue(data.zipCode);
      this.form.get('state').patchValue(data.state ?? null);
      this.form.get('city').patchValue(data.city ?? null);
      this.form.get('phone1').patchValue(data.phone);

      if(this.registrationAccess == true){
      ...
      // this.form.get('email').patchValue(data.email);
      this.form.get('status').patchValue(data.status ?? null);
      this.edit = true;
      this.getCityList();
    });
  }
  ...
  updateSubmit() {
    var request1 = {
      ...
      ownerSkillDomainEntity: {
        domainId: this.form.get('businessDomain').value??null
      },
      ...
      registrationAccess:this.registrationAccess,
      status:  this.form.value.status == 'true' ? true : false,
    };
    this.superAdminservice.updateClientDetails1(request1).subscribe(
      (response) => {
        Swal.fire({
          position: 'center',
          icon: 'success',
  ...
  onSubmit(): void {
    ...
    if (this.form.invalid) {
      ...
      var request = {
        skillSeekerName: this.form.value.businessName,
        ownerSkillDomainEntity: {
          domainId:parseInt(this.form.get('businessDomain').value) ?? null
        },
        primaryContactFullName: this.form.value.firstName + ' ' + this.form.value.lastName,
        taxIdBusinessLicense: this.form.value.businessNumber,
        addressLine1: this.form.value.addressline1,
        ...
        active: this.form.value.status == 'true' ? true : false,
      };
